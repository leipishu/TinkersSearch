...
public class FloatingSearchPanel extends AbstractWidget {
    ...
    // 1.20.1 中父类 x/y/width/height 是 private，因此这里自有字段
    ...
    public FloatingSearchPanel() {
        super(0, 0, PANEL_WIDTH, 100, Component.literal("Search Panel"));
        this.visible = false;
        ...
        this.interactionHandler = new PanelInteractionHandler(
                this, this::refreshMoltenFluids, this::onFluidClicked);
    ...
    public int getPanelX() { return this.panelX + animationManager.getAnimationOffset(); }
    public int getPanelY() { return this.panelY; }
    ...
    public int getActualPanelX() { return this.panelX + animationManager.getAnimationOffset(); }
    ...
    public int[] getTabButtonPosition() {
        ...
        int btnX = expanded ? panelX + panelWidth - 1 : 0;
        if (btnX < 0) btnX = 0;
    ...
    // ===== Setters =====
    public void setPanelX(int x) { this.panelX = x; }
    public void setPanelY(int y) { this.panelY = y; }
    public void setPanelWidth(int width) { this.panelWidth = width; }
    public void setPanelHeight(int height) { this.panelHeight = height; }
    public void setVisibleInternal(boolean visible) {
        this.isVisible = visible;
        this.visible = visible;
    }
    public void setActuallyVisible(boolean v) { this.isActuallyVisible = v; }
    ...
    public boolean isPointInsidePanel(double mouseX, double mouseY) {
        ...
        return mouseX >= actualX && mouseX <= actualX + this.panelWidth &&
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        int actualX = getPanelX();
        if (mouseX < actualX || mouseX > actualX + this.panelWidth ||
                mouseY < this.panelY || mouseY > this.panelY + this.panelHeight) return false;
        ...
        if (tab == Tab.MATERIALS) {
            int startY = this.panelY + layoutCalculator.getAllMaterialsContentY();
        ...
        int favStartY = this.panelY + layoutCalculator.getFavoriteAreaStartY();
        ...
        int smelteryStartY = this.panelY + layoutCalculator.getSmelteryAreaStartY();
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (!(mouseX >= px && mouseX <= px + pw && mouseY >= py && mouseY <= py + this.panelHeight)) return false;

        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT && renderer.beginScrollBarDrag(mouseX, mouseY)) return true;
        ...
        if (dataManager.isAlloyMode() && renderer.isClickingResultName((int) mouseX, (int) mouseY)) {
            ...
            if (!registryNames.isEmpty() || !displayNames.isEmpty()) {
                ...
                if (matchedMaterial != null) {
                    ...
                    dataManager.resetAlloyResultsScrollOffset();  // ★ 切换材料时清结果滚动
        ...
        if (interactionHandler.handleSearchBoxClick(mouseX, mouseY)) return true;
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT && renderer.isDraggingScrollBar()) {
    ...
    public boolean isDraggingScrollBar() { return renderer.isDraggingScrollBar(); }
    public void handleMouseDrag(double mouseX, double mouseY) { renderer.updateScrollBarDrag(mouseY); }
    public void handleMouseRelease() { renderer.endScrollBarDrag(); }
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        renderer.renderButton(graphics, mouseX, mouseY, partialTick);
    }
            ...
            if (mouseX >= cardX && mouseX <= cardX + cardW &&
                ...
                if (areaKind != AreaKind.SMELTERY) {
                    ...
                    for (FluidStack fs : dataManager.getAllFluids()) {
                        ...
                        if (fsRl != null && fluidRl != null && fsRl.equals(fluidRl)) {
    ...
    private boolean handleAlloyCardClick(double mouseX, double mouseY, int button) {
        ...
        if (alloyHandler.getSelectedMaterial() == null) {
            List<FluidStack> materials = alloyHandler.getFilteredMaterials();
            int startY = py + PanelConfig.CARDS_START_Y;
            int endY = py + this.panelHeight - 4;
            ...
            for (int i = 0; i < materials.size(); i++) {
                ...
                if (mouseX >= cardX && mouseX <= cardX + cardW &&
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        int currentTemp = getCurrentSmelteryTemperature();
                        alloyHandler.refreshTemperature(currentTemp);
                        dataManager.resetAlloyResultsScrollOffset();   // ★ 选中新材料时清结果滚动
