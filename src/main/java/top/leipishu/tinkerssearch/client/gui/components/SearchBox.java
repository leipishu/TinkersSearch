...
public class SearchBox {
    ...
    private Component hintText = new TextComponent("");
    ...
    public void setHintText(Component hintText) {
        this.hintText = hintText != null ? hintText : new TextComponent("");
    }
    ...
    public void render(PoseStack ps, int mouseX, int mouseY, Font font) {
        ...
        GuiComponent.fill(ps, x + width - 1, y, x + width, y + height, border);

        int textX = x + 4;
        int textY = y + 4;

        if (text.isEmpty()) {
            font.draw(ps, hintText, textX, textY, style.hintColor);
        } else {
            ...
            font.draw(ps, before, textX, textY, style.textColor);
            font.draw(ps, after, textX + beforeWidth, textY, style.textColor);

            if (focused && (System.currentTimeMillis() / 500 % 2 == 0)) {
                int cursorX = textX + beforeWidth;
                if (cursorX < x + width - 2) {
                    GuiComponent.fill(ps, cursorX, y + 2, cursorX + 1, y + height - 2,
                            style.cursorColor);
                }
            }
        }
        ...
        if (!text.isEmpty()) {
            ...
            GuiComponent.fill(ps, clearX, clearY, clearX + size, clearY + size,
                    style.clearButtonBgColor);
            font.draw(ps, "\u00a7f\u2715", clearX + 2, clearY + 1, style.clearButtonTextColor);
        }
    }
