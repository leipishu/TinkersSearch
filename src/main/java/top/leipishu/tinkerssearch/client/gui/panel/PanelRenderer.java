...
public class PanelRenderer {
    ...
    public int calculateActualCardHeight(AlloyResultCalculator.AlloyChainResult result, Font font, int cardWidth) {
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            for (FluidStack availableFs : dataManager.getAllFluids()) {
                ...
                if (fsRl != null && fsRl.equals(availRl)) {
    ...
    public void renderButton(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        renderTabBar(graphics, px, py, pw, mouseX, mouseY, font);
        renderRefreshButton(graphics, px, py, mouseX, mouseY, font);

        Tab tab = dataManager.getCurrentTab();
        if (tab == Tab.ALLOY) {
            renderAlloySearchBox(graphics, px, py, pw, mouseX, mouseY, font);
        } else {
            renderSearchBox(graphics, px, py, pw, mouseX, mouseY, font);
        }

        if (tab == Tab.SMELTERY) {
            renderSmelteryContent(graphics, px, py, pw, ph, mouseX, mouseY, font);
        } else if (tab == Tab.MATERIALS) {
            renderMaterialsContent(graphics, px, py, pw, ph, mouseX, mouseY, font);
        } else {
    ...
    private void renderTabBar(GuiGraphics graphics, int px, int py, int pw,
                              int mouseX, int mouseY, Font font) {
        graphics.fill(px + 1, py + 1, px + pw - 2, py + TITLE_BAR_HEIGHT, 0xFF2A2A2A);
        graphics.drawString(font, "§6Tinker's Search", px + 5, py + 5, 0xFFFFFF);

        Tab[] tabs = Tab.values();
        String[] labels = {
                Component.translatable("gui.tinkerssearch.tab.smeltery").getString(),
                Component.translatable("gui.tinkerssearch.tab.materials").getString(),
                Component.translatable("gui.tinkerssearch.tab.alloy").getString()
        };
        ...
        for (int i = 0; i < TAB_COUNT; i++) {
            ...
            if (isActive) {
                graphics.fill(tabX, tabY + tabH - 1, tabX + tabW, tabY + tabH, 0xFFFFAA00);
            }

            String label = labels[i];
            int textColor = isActive ? 0xFFFFDD77 : (isHover ? 0xFFDDBB55 : 0xFFAA8844);
            int textW = font.width(label);
            graphics.drawString(font, label, tabX + (tabW - textW) / 2,
    ...
    private void renderSearchBox(GuiGraphics graphics, int px, int py, int pw,
                                 int mouseX, int mouseY, Font font) {
    ...
    private void renderSmelteryContent(GuiGraphics graphics, int px, int py, int pw, int ph,
                                       int mouseX, int mouseY, Font font) {
        ...
        if (!dataManager.getDisplayedFavoriteFluids().isEmpty()) {
            ...
            graphics.drawString(font, "§6" + Component.translatable("gui.tinkerssearch.favorites").getString(),
            ...
            if (favHeight > 0) {
                ...
                if (scissorOk) {
                    try {
                        RenderSystem.disableDepthTest();
                        renderFavoriteCards(graphics, px, pw, mouseX, mouseY, font, favStartY, favHeight);
                    } finally {
                ...
                renderScrollBar(graphics, px, favStartY, favHeight, pw,
        ...
        graphics.drawString(font, "§e" + Component.translatable("gui.tinkerssearch.smeltery").getString(),
        ...
        if (smelteryHeight > 0) {
            ...
            if (scissorOk) {
                try {
                    RenderSystem.disableDepthTest();
                    renderSmelteryCards(graphics, px, pw, smelteryStartY, smelteryHeight, mouseX, mouseY, font);
                } finally {
                    ScissorHelper.disableScissor();
                    RenderSystem.enableDepthTest();
                }
            ...
            renderScrollBar(graphics, px, smelteryStartY, smelteryHeight, pw,
                    dataManager.getScrollOffset(), dataManager.getMaxScrollOffset(),
                    smelteryScrollBar, mouseX, mouseY);
        }
    }

    private void renderSmelteryCards(GuiGraphics graphics, int px, int pw, int areaStartY, int areaHeight,
                                     int mouseX, int mouseY, Font font) {
        ...
        if (dataManager.getDisplayedFluids().isEmpty()) {
            ...
            graphics.drawString(font, msg, px + 5, areaStartY + 10, 0x666666);
            return;
        }
        ...
        for (int i = 0; i < dataManager.getDisplayedFluids().size(); i++) {
            ...
            drawSingleCard(graphics, cardX, cardY, cardW, cardH, fluid, hover, font, AreaKind.SMELTERY);
        }
    }

    private void renderFavoriteCards(GuiGraphics graphics, int px, int pw,
        ...
        for (int i = 0; i < dataManager.getDisplayedFavoriteFluids().size(); i++) {
            ...
            boolean hover = isHovered(cardX, cardY, cardW, cardH, mouseX, mouseY);
            drawSingleCard(graphics, cardX, cardY, cardW, cardH, fluid, hover, font, AreaKind.FAVORITE);
        }
    }

    // ==================== 材料 Tab ====================

    private void renderMaterialsContent(GuiGraphics graphics, int px, int py, int pw, int ph,
                                        int mouseX, int mouseY, Font font) {
        int cardAreaW = pw - 10 - SCROLL_BAR_WIDTH - SCROLL_BAR_PADDING;
        ...
        if (dataManager.getDisplayedAllMaterials().isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.no_materials").getString(),
        ...
        if (scissorOk) {
            try {
                RenderSystem.disableDepthTest();
                renderAllMaterialsCards(graphics, px, pw, startY, endY, mouseX, mouseY, font);
            } finally {
                ScissorHelper.disableScissor();
                RenderSystem.enableDepthTest();
            }
        ...
        renderScrollBar(graphics, px, startY, height, pw,
                dataManager.getAllMaterialsScrollOffset(), dataManager.getMaxAllMaterialsScrollOffset(),
                allMaterialsScrollBar, mouseX, mouseY);
    }

    private void renderAllMaterialsCards(GuiGraphics graphics, int px, int pw, int areaStartY, int areaEndY,
                                         int mouseX, int mouseY, Font font) {
        ...
        for (int i = 0; i < dataManager.getDisplayedAllMaterials().size(); i++) {
            ...
            boolean hover = isHovered(cardX, cardY, cardW, cardH, mouseX, mouseY);
            drawSingleCard(graphics, cardX, cardY, cardW, cardH, fluid, hover, font, AreaKind.ALL_MATERIALS);
        }
    }

    // ==================== 卡片 ====================

    private void drawSingleCard(GuiGraphics graphics, int x, int y, int w, int h, FluidStack fluid,
        ...
        CardBackground.draw(graphics, x, y, w, h, bg, border);
        ...
        if (hover && existsInSmeltery) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.click_move").getString(),
                    x + 4, y + h - 10, 0x666666);
        }
    }
    ...
    private void renderScrollBar(GuiGraphics graphics, int px, int areaStartY, int areaHeight, int pw,
        ...
        bar.render(graphics, mouseX, mouseY);
    }
    ...
    private void renderAlloyContent(GuiGraphics graphics, int px, int py, int pw, int ph,
                                    int mouseX, int mouseY, Font font) {
        ...
        graphics.drawString(font, "§b" + selectedName + " §7"
                        + Component.translatable("gui.tinkerssearch.alloy_results").getString() + ":",
        ...
        if (results.isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.alloy_no_recipes").getString(),
    ...
    private void renderAlloyMaterials(GuiGraphics graphics, int px, int py, int pw,
        ...
        if (materials.isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.no_match").getString(),
            ...
            try {
                ...
                for (int i = 0; i < materials.size(); i++) {
                    ...
                    boolean selected = alloyHandler.getSelectedMaterial() != null &&
                            isSameFluid(alloyHandler.getSelectedMaterial(), fluid);
    ...
    /** 通过 ForgeRegistries 比较两个 FluidStack 是否同一流体（1.20.1 兼容）。 */
    private boolean isSameFluid(FluidStack a, FluidStack b) {
        ResourceLocation aRl = ForgeRegistries.FLUIDS.getKey(a.getFluid());
        ResourceLocation bRl = ForgeRegistries.FLUIDS.getKey(b.getFluid());
        if (aRl == null || bRl == null) return a.getFluid() == b.getFluid();
        return aRl.equals(bRl);
    }
    ...
    private int drawAlloyRecipeCard(GuiGraphics graphics, int px, int pw, int y,
        ...
        CardBackground.draw(graphics, x, y, w, cardH, bg, border);
        ...
        if (result.getNext() != null && lineY < y + cardH - 4) {
            ...
            if (hasMissing) {
                ...
                nextMissingInfo.append("§c  ")
                        .append(Component.translatable("gui.tinkerssearch.alloy_next_missing").getString())
                        .append(": ");
