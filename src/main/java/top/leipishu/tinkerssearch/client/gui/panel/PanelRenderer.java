...
public class PanelRenderer {
    ...
    public int calculateActualCardHeight(AlloyResultCalculator.AlloyChainResult result, Font font, int cardWidth) {
        ...
        ingredients.append(Component.translatable("gui.tinkerssearch.alloy_ingredients").getString());
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            for (FluidStack availableFs : dataManager.getAllFluids()) {
                ResourceLocation availableRl = ForgeRegistries.FLUIDS.getKey(availableFs.getFluid());
                if (availableRl != null && availableRl.equals(fsRl)) {
    ...
    public void renderButton(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // GuiComponent.fill → graphics.fill
        graphics.fill(px, py, px + pw, py + ph, 0xFF1A1A1A);

        graphics.fill(px, py, px + 1, py + ph, 0x33FFFFFF);
        graphics.fill(px + pw - 1, py, px + pw, py + ph, 0x22FFFFFF);
        graphics.fill(px, py, px + pw, py + 1, 0x22FFFFFF);
        graphics.fill(px, py + ph - 1, px + pw, py + ph, 0x22FFFFFF);

        renderTitleBar(graphics, px, py, pw, font);
        renderRefreshButton(graphics, px, py, mouseX, mouseY, font);

        if (dataManager.isAlloyMode()) {
            renderAlloySearchBox(graphics, px, py, pw, font);
        } else {
            renderSearchBox(graphics, px, py, pw, font);
        }

        if (dataManager.isAlloyMode()) {
            renderAlloyContent(graphics, px, py, pw, ph, mouseX, mouseY, font);
        } else {
            renderNormalContent(graphics, px, py, pw, ph, mouseX, mouseY, font);
        }
    ...
    private void renderTitleBar(GuiGraphics graphics, int px, int py, int pw, Font font) {
        graphics.fill(px + 1, py + 1, px + pw - 2, py + TITLE_BAR_HEIGHT, 0xFF2A2A2A);
        String title = dataManager.isAlloyMode() ?
                Component.translatable("gui.tinkerssearch.alloy_query").getString() :
                "Tinker's Search";
        // Font.draw → graphics.drawString
        graphics.drawString(font, (dataManager.isAlloyMode() ? "§b" : "§6") + title, px + 5, py + 5, 0xFFFFFF);
    }

    private void renderRefreshButton(GuiGraphics graphics, int px, int py, int mouseX, int mouseY, Font font) {
        ...
        graphics.fill(btnX, btnY, btnX + REFRESH_BTN_W, btnY + REFRESH_BTN_H, color);
        graphics.fill(btnX, btnY, btnX + REFRESH_BTN_W, btnY + 1, 0xFF666666);
        graphics.fill(btnX, btnY + REFRESH_BTN_H - 1, btnX + REFRESH_BTN_W, btnY + REFRESH_BTN_H, 0xFF666666);
        graphics.fill(btnX, btnY, btnX + 1, btnY + REFRESH_BTN_H, 0xFF666666);
        graphics.fill(btnX + REFRESH_BTN_W - 1, btnY, btnX + REFRESH_BTN_W, btnY + REFRESH_BTN_H, 0xFF666666);

        graphics.drawString(font, Component.translatable("gui.tinkerssearch.refresh"), btnX + 4, btnY + 3, 0xCCCCCC);
    }

    private void renderSearchBox(GuiGraphics graphics, int px, int py, int pw, Font font) {
        ...
        if (keyword.isEmpty()) {
            graphics.drawString(font, Component.translatable("gui.tinkerssearch.search_hint"), boxX + 4, boxY + 4, 0x666666);
        } else {
            ...
            graphics.drawString(font, beforeCursor, boxX + 4, boxY + 4, 0xFFFFFF);
            graphics.drawString(font, afterCursor, boxX + 4 + beforeWidth, boxY + 4, 0xFFFFFF);

            if (focused && (System.currentTimeMillis() / 500 % 2 == 0)) {
                int cursorX = boxX + 4 + beforeWidth;
                if (cursorX < boxX + boxW - 2) {
                    graphics.fill(cursorX, boxY + 2, cursorX + 1, boxY + boxH - 2, 0xFFFFFFFF);
                }
            }
        }
        ...
        graphics.drawString(font, countStr, px + pw - 35, boxY + 4, 0x888888);

        int lineY = boxY + boxH + 4;
        graphics.fill(px + 5, lineY, px + pw - 5 - SCROLL_BAR_WIDTH - SCROLL_BAR_PADDING, lineY + 1, 0xFF333333);
    }

    private void renderAlloySearchBox(GuiGraphics graphics, int px, int py, int pw, Font font) {
        boolean focused = interactionHandler.isSearchBoxFocused();
        String keyword = interactionHandler.getSearchKeyword();
        int cursorPos = interactionHandler.getCursorPosition();
        ...
        if (keyword.isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.alloy_search_hint").getString(), boxX + 4, boxY + 4, 0x666666);
        } else {
            String beforeCursor = keyword.substring(0, cursorPos);
            String afterCursor = keyword.substring(cursorPos);
            int beforeWidth = font.width(beforeCursor);
        ...
        int lineY = boxY + boxH + 4;
        graphics.fill(px + 5, lineY, px + pw - 5 - SCROLL_BAR_WIDTH - SCROLL_BAR_PADDING, lineY + 1, 0xFF333366);
    }

    private void renderNormalContent(GuiGraphics graphics, int px, int py, int pw, int ph, int mouseX, int mouseY, Font font) {
        if (!dataManager.getDisplayedFavoriteFluids().isEmpty()) {
            int favLabelY = py + CARDS_START_Y;
            graphics.drawString(font, "§6" + Component.translatable("gui.tinkerssearch.favorites").getString(), px + 5, favLabelY, 0xFFFFFF);
            ...
            if (favAreaHeight > 0) {
                ...
                if (scissorOk) {
                    try {
                        RenderSystem.disableDepthTest();
                        renderFavoriteCards(graphics, px, py, pw, ph, mouseX, mouseY, font, favStartY, favAreaHeight);
                    } finally {
                ...
                renderScrollBar(graphics, px, favStartY, favAreaHeight, pw, dataManager.getFavScrollOffset(), dataManager.getMaxFavScrollOffset());
            }

            int sepY = favStartY + favAreaHeight + SECTION_SPACING;
            graphics.fill(px + 5, sepY, px + pw - 5 - SCROLL_BAR_WIDTH - SCROLL_BAR_PADDING, sepY + 1, 0xFF444444);

            int smelterLabelY = sepY + SECTION_SPACING;
            graphics.drawString(font, "§e" + Component.translatable("gui.tinkerssearch.smeltery").getString(), px + 5, smelterLabelY, 0xFFFFFF);
        } else {
        ...
        if (clipHeight > 0) {
            ...
            if (scissorOk) {
                try {
                    RenderSystem.disableDepthTest();
                    renderCards(graphics, px, py, pw, ph, mouseX, mouseY, font);
                } finally {
                    ScissorHelper.disableScissor();
                    RenderSystem.enableDepthTest();
                }
            ...
            renderScrollBar(graphics, px, clipStartY, clipHeight, pw, dataManager.getScrollOffset(), dataManager.getMaxScrollOffset());
        }
    }

    private void renderCards(GuiGraphics graphics, int px, int py, int pw, int ph, int mouseX, int mouseY, Font font) {
        ...
        if (dataManager.getDisplayedFluids().isEmpty()) {
            ...
            graphics.drawString(font, msg, px + 5, startY + dataManager.getScrollOffset() + 10, 0x666666);
            return;
        }
        ...
        for (int i = 0; i < dataManager.getDisplayedFluids().size(); i++) {
            ...
            drawSingleCard(graphics, cardX, cardY, cardW, cardH, fluid, isHover, font, false);
        }
    }

    private void renderFavoriteCards(GuiGraphics graphics, int px, int py, int pw, int ph, int mouseX, int mouseY, Font font, int areaStartY, int areaHeight) {
        int startY = areaStartY - dataManager.getFavScrollOffset();
        int endY = areaStartY + areaHeight;

        if (dataManager.getDisplayedFavoriteFluids().isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.no_favorites").getString(), px + 5, areaStartY + 10, 0x666666);
            return;
        }
        ...
        for (int i = 0; i < dataManager.getDisplayedFavoriteFluids().size(); i++) {
            ...
            boolean isHover = isHovered(cardX, cardY, cardW, cardH, mouseX, mouseY);
            drawSingleCard(graphics, cardX, cardY, cardW, cardH, fluid, isHover, font, true);
        }
    }

    private void drawSingleCard(GuiGraphics graphics, int x, int y, int w, int h, FluidStack fluid, boolean hover, Font font, boolean isFavorite) {
        ...
        if (isBottom) {
            ...
            graphics.fill(x, y, x + w, y + 1, green);
            graphics.fill(x, y + h - 1, x + w, y + h, green);
            graphics.fill(x, y, x + 1, y + h, green);
            graphics.fill(x + w - 1, y, x + w, y + h, green);
        } else {
        ...
        SmelteryDataHelper.drawFluidIcon(graphics, iconX, iconY, fluid, iconSize);
        ...
        graphics.drawString(font, truncatedName, textX, y + 4, nameColor);

        if (isFavorite && !existsInSmeltery) {
            String locked = Component.translatable("gui.tinkerssearch.locked").getString();
            graphics.drawString(font, "§8" + locked, textX, y + 18, 0x888888);
        } else {
            ...
            graphics.drawString(font, "§8" + amtStr, textX, y + 18, 0x888888);
        }
        ...
        graphics.drawString(font, star, starX, starY, starColor);

        if (hover && existsInSmeltery) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.click_move"), x + 4, y + h - 10, 0x666666);
        }
    }

    private void renderScrollBar(GuiGraphics graphics, int px, int areaStartY, int areaHeight, int panelWidth,
        ...
        graphics.fill(barX, barY, barX + SCROLL_BAR_WIDTH, barY + barH, 0x33FFFFFF);
        ...
        graphics.fill(barX, thumbY, barX + SCROLL_BAR_WIDTH, thumbY + thumbH, 0x99FFFFFF);
    }

    // ==================== 合金模式渲染 ====================

    private void renderAlloyContent(GuiGraphics graphics, int px, int py, int pw, int ph, int mouseX, int mouseY, Font font) {
        int startY = py + CARDS_START_Y + 18;
        int endY = py + ph - 4;

        if (alloyHandler.getSelectedMaterial() == null) {
            renderAlloyMaterials(graphics, px, py, pw, mouseX, mouseY, font, startY, endY);
            return;
        }

        int contentY = startY;

        String backText = Component.translatable("gui.tinkerssearch.alloy_back").getString();
        graphics.drawString(font, "§7" + backText, px + 5, contentY, 0xCCCCCC);
        contentY += 14;

        FluidStack selected = alloyHandler.getSelectedMaterial();
        String selectedName = selected.getDisplayName().getString().replace("Molten ", "");
        graphics.drawString(font, "§b" + selectedName + " §7" + Component.translatable("gui.tinkerssearch.alloy_results").getString() + ":", px + 5, contentY, 0xFFFFFF);
        contentY += 12;
        ...
        if (results.isEmpty()) {
            graphics.drawString(font, Component.translatable("gui.tinkerssearch.alloy_no_recipes").getString(), px + 5, contentY, 0x666666);
            return;
        }
        ...
        if (scissorOk) {
            try {
                ...
                for (AlloyResultCalculator.AlloyChainResult result : results) {
                    cardY = drawAlloyRecipeCard(graphics, px, pw, cardY, result, currentTemp, mouseX, mouseY, font);
                    cardY += 6;
                    if (cardY > endY) break;
                }
            } finally {
        ...
        if (maxOffset > 0) {
            renderScrollBar(graphics, px, cardStartY, cardAreaHeight, pw, alloyScrollOffset, maxOffset);
        }
    }

    private void renderAlloyMaterials(GuiGraphics graphics, int px, int py, int pw, int mouseX, int mouseY, Font font, int startY, int endY) {
        List<FluidStack> materials = alloyHandler.getFilteredMaterials();

        String title = Component.translatable("gui.tinkerssearch.alloy_materials").getString();
        graphics.drawString(font, "§7" + title + " §8(" + materials.size() + ")", px + 5, startY, 0xCCCCCC);

        int cardStartY = startY + 18;
        int cardAreaHeight = endY - cardStartY;

        if (materials.isEmpty()) {
            graphics.drawString(font, "§7" + Component.translatable("gui.tinkerssearch.no_match"), px + 5, cardStartY + 10, 0x666666);
            return;
        }
        ...
        if (scissorOk) {
            try {
                ...
                for (int i = 0; i < materials.size(); i++) {
                    ...
                    ResourceLocation fluidRl = ForgeRegistries.FLUIDS.getKey(fluid.getFluid());
                    boolean selected = alloyHandler.getSelectedMaterial() != null &&
                            ForgeRegistries.FLUIDS.getKey(alloyHandler.getSelectedMaterial().getFluid()).equals(fluidRl);

                    drawAlloyMaterialCard(graphics, cardX, cardY, cardW, cardH, fluid, hover, selected, font);
                }
            } finally {
                ScissorHelper.disableScissor();
                RenderSystem.enableDepthTest();
    ...
    private void drawAlloyMaterialCard(GuiGraphics graphics, int x, int y, int w, int h,
        ...
        graphics.drawString(font, truncated, textX, y + (h - font.lineHeight) / 2 + 1, 0xFFFFFF);
    }

    // ==================== 合金卡片绘制 ====================

    private int drawAlloyRecipeCard(GuiGraphics graphics, int px, int pw, int y,
        ...
        graphics.fill(x, y, x + w, y + cardH, bg);

        int border = feasible ? 0xFF44FF44 : 0xFFFF8800;
        graphics.fill(x, y, x + w, y + 1, border);
        graphics.fill(x, y + cardH - 1, x + w, y + cardH, border);
        graphics.fill(x, y, x + 1, y + cardH, border);
        graphics.fill(x + w - 1, y, x + w, y + cardH, border);

        // ===== 第1行：状态 =====
        String status = feasible ?
                "§a✔ " + Component.translatable("gui.tinkerssearch.alloy_feasible").getString() :
                "§c✘ " + Component.translatable("gui.tinkerssearch.alloy_infeasible").getString();
        graphics.drawString(font, status, x + 6, y + 3, 0xFFFFFF);

        // ===== 第1行右侧：温度 =====
        String tempStr = feasibility.isTemperatureOk() ?
                "§a" + currentTemp + "°C" :
                "§c" + currentTemp + "°C §7/§e" + feasibility.getRequiredTemp() + "°C";
        graphics.drawString(font, tempStr, x + w - font.width(tempStr) - 6, y + 3, 0xFFFFFF);

        // ===== 分割线 =====
        int lineY = y + 16;
        graphics.fill(x + 4, lineY, x + w - 4, lineY + 1, 0x44FFFFFF);
        lineY += 6;
        ...
        if (eqIndex >= 0) {
            ...
            graphics.drawString(font, beforePart, x + 6, lineY, 0xCCCCCC);
            ...
            if (arrowIdx >= 0) {
                ...
                graphics.drawString(font, "§7" + restPart, restX, lineY, 0xCCCCCC);
            }
            ...
            graphics.drawString(font, "§7" + chainStr, x + 6, lineY, 0xCCCCCC);
        }
        ...
        if (!missing.isEmpty()) {
            ...
            sb.append(Component.translatable("gui.tinkerssearch.alloy_missing_prefix").getString());
            sb.append(": ");
            ...
            graphics.drawString(font, sb.toString(), x + 6, lineY, 0xCCCCCC);
            lineY += 12;
        }
        ...
        if (result.getNext() == null) {
            ...
            if (maxTimes > 0) {
                String timesText = Component.translatable("gui.tinkerssearch.alloy_times", maxTimes).getString();
                graphics.drawString(font, "§e" + timesText, x + 6, lineY, 0xCCCCCC);
            } else {
                String unableText = Component.translatable("gui.tinkerssearch.alloy_unable").getString();
                graphics.drawString(font, "§c" + unableText, x + 6, lineY, 0xCCCCCC);
            }
            lineY += 12;
        }
        ...
        if (lineY < y + cardH - 4) {
            ...
            for (String line : wrappedLines) {
                if (lineY < y + cardH - 4) {
                    graphics.drawString(font, line, x + 6, lineY, 0x888888);
                    lineY += 12;
                }
            }
        }
        ...
        if (result.getNext() != null && lineY < y + cardH - 4) {
            ...
            nextInfo.append(" §7| ")
                    .append(Component.translatable("gui.tinkerssearch.alloy_temp_short").getString())
                    .append(": ");
            ...
            graphics.drawString(font, nextInfo.toString(), x + 6, lineY, 0x888888);
            lineY += 12;
        }
        ...
        if (result.getNext() != null && lineY < y + cardH - 4) {
            ...
            if (hasMissing) {
                ...
                nextMissingInfo.append(Component.translatable("gui.tinkerssearch.alloy_next_missing").getString());
                ...
                graphics.drawString(font, nextMissingStr, x + 6, lineY, 0xCCCCCC);
            }
        }
