...
public class CastingRecipeHelper {
    ...
    /** ★ 部件类型引用（1.19.2 保留）。{@code FluidPartDataCache} 会 import 它。 */
    ...
    public static List<CastingInfo> getCastingRecipesForFluid(FluidStack fs) {
        return getAllCastingOutputs(fs);
    }

    public static boolean hasCastingRecipes(FluidStack fs) {
        return !getAllCastingOutputs(fs).isEmpty();
    }
    ...
    public static List<CastingInfo> getDirectPartCastingRecipes(FluidStack fs) {
        return getAnyPartCastingRecipes(fs);
    }
