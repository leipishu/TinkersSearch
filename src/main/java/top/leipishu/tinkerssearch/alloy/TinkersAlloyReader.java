 ...
 * 合金配方读取器 - 支持多种配方来源
 *
 * 1.20.1 版本：保留直接 AlloyRecipe/FluidIngredient 导入
 * 兼容 1.20.1 的匠魂结构（AlloyRecipe.inputs: List<AlloyIngredient> record）
 *
 * 读取策略（按优先级）：
...
public class TinkersAlloyReader {
    ...
    private static List<AlloyRecipeData> loadFromRecipeType() {
        ...
        try {
            ...
            // 注意：在 1.20.1 中，getRecipesFor 需要 RecipeType 和 Level
            try {
                        ...
                        for (Map.Entry<?, ?> entry : recipesMap.entrySet()) {
                            ...
                            if (value instanceof Map) {
                                ...
                                for (Map.Entry<?, ?> innerEntry : innerMap.entrySet()) {
                                    ...
                                    // ✅ 1.20.1：使用类名反射检测（兼容 AlloyRecipe 子类）
            ...
            try {
                ...
                if (ingredientsObj instanceof List) {
                    ...
                    for (Object ing : ingredientList) {
                        // ✅ 1.20.1：使用反射提取 FluidIngredient / AlloyIngredient
    ...
    // ============================================================
    // ===== 核心：解析 AlloyRecipe（兼容 1.20.1 结构） =========
    // ============================================================
     ...
     * 解析匠魂合金配方
     *
     * 1.20.1 AlloyRecipe 结构：
     * - inputs: List<AlloyIngredient>（record 类型，结构为 { FluidIngredient fluid, boolean catalyst }）
     * - output/result: FluidStack
     * - temperature: int
     */
    @SuppressWarnings("unchecked")
    private static AlloyRecipeData parseAlloyRecipe(Object recipeObj, ResourceLocation id) {
        try {
            ...
            for (Object ingredient : inputs) {
                ...
                // ✅ 1.20.1：使用 extractAlloyIngredient 处理 AlloyIngredient record 或 FluidIngredient
    ...
    private static void addFluidToSet(FluidStack fs, Set<ResourceLocation> set) {
        ...
        // ✅ 1.20.1：使用 ForgeRegistries 获取注册名
