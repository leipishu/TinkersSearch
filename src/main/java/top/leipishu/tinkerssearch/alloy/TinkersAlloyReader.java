 ...
 * 合金配方读取器（1.20.1）。
 *
 * <p>保留 1.20.1 的 {@code instanceof AlloyRecipe} / {@code AlloyIngredient record}
 * 支持；同时恢复 1.19.2 的：
 * <ul>
 *   <li>{@code cachedFluidsWithRecipes} 字段</li>
 *   <li>{@link #getFluidsWithRecipes()} —— 收集所有"出现在任意配方中"的流体</li>
 *   <li>{@link #getAllSmelteryFluids()} —— "注册表 ∩ 有配方"过滤</li>
 * </ul>
 */
public class TinkersAlloyReader {
    ...
    private static AlloyRecipeData parseAlloyRecipe(Object recipeObj, ResourceLocation id) {
        try {
            ...
            for (Object ingredient : inputs) {
                ...
                AlloyRecipeData.FluidIngredientData data = extractAlloyIngredient(ingredient);
    ...
    /**
     * 提取 AlloyIngredient / FluidIngredient / FluidStack 中的流体信息。
     *
     * <p>1.20.1：支持 {@code AlloyIngredient record} 结构
     * （{@code getFluid() → FluidIngredient} / {@code getCatalyst()}）。
     */
        ...
        try {
            ...
            if (className.contains("AlloyIngredient")) {
                ...
                if (fluidIngredient != null) {
                    return extractFluidFromIngredient(fluidIngredient);
                }
            }
    ...
    /** 从 FluidIngredient 中提取 FluidStack（反射方式）。 */
    private static AlloyRecipeData.FluidIngredientData extractFluidFromIngredient(Object ingredient) {
        try {
            ...
            try {
                ...
                if (obj instanceof List) fluids = (List<?>) obj;
            } catch (Exception ignored) {}
            ...
            if (fluids == null || fluids.isEmpty()) return null;

            Object first = fluids.get(0);
            if (!(first instanceof FluidStack)) return null;
     ...
     * <p><b>1.19.2 逻辑</b>：
     * <ol>
     *   <li>先扫描整个流体注册表，收集所有匠魂 / molten / liquid / metal 流体</li>
     *   <li>再用"出现在任意配方中"的集合过滤</li>
     * </ol>
     *
     * <p>这与"仅从合金配方提取"的关键区别：会额外包含参与浇筑 / 熔化 /
     * 冶炼等非合金配方但未参与合金配方的流体。
     */
     ...
     *
     * <p>1.19.2 保留方法——之前 1.20.1 版本被删除，导致材料列表退化成
     * 只从合金配方提取。
     */
    ...
    public static void clearCache() {
        ...
        cachedAllMaterials = null;
        cachedFluidsWithRecipes = null;   // ★ 保留
        recipeCache.clear();
        cacheTime = 0;
        log("Cache cleared");
    }
